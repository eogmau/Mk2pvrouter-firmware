 ...
 *  This is to delay start-up while other conrol systems respond to changing conditions.
 *  When an export situation ends (i.e. no loads able to consume power), the Mk2 will 
 *  deactivate its load after the normal (0.5 Wh) delay.
 * 
 *      Robin Emley
...
{  
  ...
  Serial.println();
  Serial.println("-------------------------------------");
  Serial.println("Sketch ID:      Mk2_bothDisplays_4_1290.ino");
  Serial.println();
...
{
  float baseline = ((float)WORKING_RANGE_IN_JOULES - 1800) / WORKING_RANGE_IN_JOULES;
  // to provide a delayed startup when surplus power first appears
  // and the standard switchoff delay when an overflow situation ends
  ...
  if (outputMode == ANTI_FLICKER)
  {
    ...
    lowerEnergyThreshold_long = 
       capacityOfEnergyBucket_long * (baseline - offsetOfEnergyThresholdsInAFmode); 
